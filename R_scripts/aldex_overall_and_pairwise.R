...
#          for taxon-level differential abundance analysis, with publication-ready
...
# -------------------------
counts_path <- "data/Bracken_SpeciesCounts.csv"
...
Bracken_SpeciesCounts <- read_csv(counts_path, show_col_types = FALSE)
chicken_metadata      <- read_csv(meta_path,   show_col_types = FALSE)

# Ensure expected column names exist
if (!"name"      %in% colnames(Bracken_SpeciesCounts)) stop("Counts CSV must contain a 'name' column with taxa.")
...
# Move names to rownames (taxa x samples)
...
if (!all(rownames(meta) %in% colnames(counts))) {
  if (all(rownames(meta) %in% rownames(counts))) {
    message("Counts appear to be samples x taxa; transposing counts.")
    counts <- t(counts)
  } else {
...
message("Counts and metadata aligned: ", nrow(counts), " taxa x ", ncol(counts), " samples.")
...
message("Filtered taxa: kept ", nrow(counts_f), " taxa (present in >= ", min_samples_present, " samples).")
...
run_aldex_on_subset <- function(counts_all, meta_all, groups_vec, comp_name, aldex_args) {
  ...
  prefixed <- res_df %>%
    relocate(taxon) %>%
    rename_with(.cols = -taxon, .fn = ~ paste0(comp_name, "_", .))
  return(prefixed)
}
...
merged_pairwise <- reduce(results_list, full_join, by = "taxon")

overall_df        <- as.data.frame(overall_res)
overall_df$taxon  <- rownames(overall_df)
keep_overall_cols <- intersect(c("kw.ep", "kw.eBH", "kw.effect"), colnames(overall_df))
if (length(keep_overall_cols) == 0) keep_overall_cols <- setdiff(colnames(overall_df), "taxon")
overall_subset <- overall_df %>% select(taxon, all_of(keep_overall_cols))

merged_all <- left_join(merged_pairwise, overall_subset, by = "taxon")
...
# Compute mean and SE per taxon x Treatment from raw counts
rawCount_stats <- Bracken_SpeciesCounts %>%
  distinct(name, .keep_all = TRUE) %>%
...
rawCount_combined_df <- rawCount_wide_combined %>% rownames_to_column(var = "taxon")
rawCount_numeric_df  <- rawCount_wide_numeric  %>% rownames_to_column(var = "taxon")

merged_all2 <- merged_all %>%
  left_join(rawCount_combined_df, by = "taxon") %>%
  left_join(rawCount_numeric_df,  by = "taxon")
...
print(head(merged_all2[, c("taxon", added_cols)], 6))

# ---------- Pairwise plots ----------
out_pdf          <- "output/aldex_pairwise_plots_and_taxon_plots.pdf"
...
for (comp in names(raw_results)) {
  ...
  pub_cols <- unique(c("taxon", "effect", "overlap", qcol_w,
  ...
  if (nrow(cand) == 0) {
    top_taxa <- res_df %>% arrange(desc(abs(effect))) %>% slice_head(n = top_n_taxa_plots) %>% pull(taxon)
    message("No candidates passed thresholds for ", comp, "; plotting top ", length(top_taxa), " taxa by effect.")
  } else {
    top_taxa <- head(cand$taxon, top_n_taxa_plots)
    message("Plotting ", length(top_taxa), " candidate taxa for ", comp)
  }
  ...
  for (tax in top_taxa) {
    class_data <- counts_long %>% filter(taxon == tax)
    ...
    barp <- ggplot(class_data, aes(x = sample, y = count, fill = Treatment)) +
      ...
      labs(title = bquote(italic(.(tax)) ~ "counts per sample (KW p =" ~
...
# -------------------- Top taxa summary --------------------
out_pdf2         <- "output/aldex_top_taxa_bar_violin.pdf"
...
for (comp in pairwise_comps) {
  ...
  tmp <- merged_all2 %>%
    select(taxon, all_of(c(cols$effect, cols$overlap, cols$q))) %>%
  ...
  if (nrow(cand) < per_comp_candidates) {
    ...
    cand <- distinct(bind_rows(cand, more), taxon, .keep_all = TRUE)
  } else {
    cand <- cand %>% slice_head(n = per_comp_candidates)
  }
  top_candidates_list[[comp]] <- cand$taxon
  message("Selected ", length(cand$taxon), " candidates for ", comp)
}
...
if (length(all_candidates) > (plot_top_n * length(pairwise_comps))) {
  ...
  rank_df <- merged_all2 %>%
    filter(taxon %in% all_candidates) %>%
  ...
  all_candidates <- rank_df$taxon[1:keep_n]
...
for (tax in all_candidates) {
  ...
  barp <- ggplot(class_data, aes(x = reorder(sample, count), y = count, fill = Treatment)) +
    ...
    scale_fill_manual(values = c("Reference diet" = "#1f77b4",
                                 "Dulce"          = "#2ca02c",
                                 "Soyabean meal"  = "#ff7f0e")) +
    labs(title = bquote(italic(.(tax)) ~ "Count per Treatment (" ~ .(kw_label) ~ ")"),
  ...
  violp <- ggplot(class_data, aes(x = Treatment, y = log_count, fill = Treatment)) +
    ...
    theme(legend.position = "none", text = element_text(size = 12),
          plot.title = element_text(hjust = 0.5)) +
    ggtitle(paste0(tax, " \u2014 group comparison")) +
...
# Publication-ready CSV for selected taxa
...
summary_cols <- unique(c("taxon", overall_cols, pairwise_summary_cols))
