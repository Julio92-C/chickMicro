# Purpose: Pheatmap of TPM-normalised ARG (CARD) profiles across samples,
#          with drug-class row annotation and treatment column annotation.
# Input:   data/genetable_normdata.csv (cols: sample, GENE, PRODUCT, RESISTANCE,
#                                       DATABASE, TPM, Treatment, ...)
# Output:  Heatmap rendered to the active graphics device.

library(readr)
library(dplyr)
library(tidyverse)
library(paletteer)
library(reshape2)
library(pheatmap)
library(stringr)

# Load dataset
genetable_normdata <- read_csv("data/genetable_normdata.csv")

# Helper functions
clean_string <- function(x) str_to_title(str_replace_all(x, "_", " "))

classify_resistance <- function(res_string) {
  if (is.null(res_string) || is.na(res_string) || res_string == "") return(NA)
  classes        <- trimws(unlist(strsplit(res_string, ";")))
  classes        <- classes[classes != ""]
  unique_classes <- unique(classes)
  if (length(unique_classes) == 1) return(unique_classes)
  if (length(unique_classes) > 1) return("Multi-drug")
  return(NA)
}

# Drug/gene name clean-up lookup applied in two places below
rename_genes <- function(df) {
  df %>%
    mutate(GENE = case_when(
      grepl("vanW_gene_in_vanB_cluster", GENE) ~ "vanWB",
      grepl("vanR_gene_in_vanB_cluster", GENE) ~ "vanRB",
      grepl("vanX_gene_in_vanB_cluster", GENE) ~ "vanXB",
      grepl("vanY_gene_in_vanB_cluster", GENE) ~ "vanYB",
      grepl("vanS_gene_in_vanB_cluster", GENE) ~ "vanSB",
      grepl("vanH_gene_in_vanB_cluster", GENE) ~ "vanHB",
      grepl("Escherichia_coli_ampC_beta-lactamase", GENE) ~ "ampC",
      grepl("Escherichia_coli_mdfA", GENE)     ~ "mdfA",
      grepl("Escherichia_coli_emrE", GENE)     ~ "emrE",
      grepl("Escherichia_coli_acrA", GENE)     ~ "acrA",
      grepl("Vibrio_anguillarum_chloramphenicol_acetyltransferase", GENE) ~ "vacA*",
      grepl("Bifidobacterium_adolescentis_rpoB_mutants_conferring_resistance_to_rifampicin", GENE) ~ "rpoB_mutants*",
      grepl("Bifidobacterium_bifidum_ileS_conferring_resistance_to_mupirocin", GENE) ~ "ileS*",
      TRUE ~ GENE
    ),
    GENE = ifelse(grepl("AAC\\(6'\\)-Ie-APH\\(2''\\)-Ia_bifunctional_protein", GENE),
                  "aac(6')-Ie/aph(2'')-Ia", GENE))
}

# Build GENE x sample TPM matrix
# NOTE: the mutate/arrange block before select() intentionally has no pipe continuation
# (the original code had a missing pipe after arrange()); this is preserved as-is.
geneTable_filtered <- genetable_normdata %>%
  filter(grepl("card", DATABASE)) %>%
  mutate(RESISTANCE = str_replace(
    RESISTANCE,
    "lincosamide;macrolide;streptogramin;streptogramin_A;streptogramin_B", "MLS"
  )) %>%
  mutate(RESISTANCE = sapply(RESISTANCE, clean_string)) %>%
  mutate(RESISTANCE = sapply(RESISTANCE, classify_resistance)) %>%
  mutate(RESISTANCE = str_replace(RESISTANCE, "Mls", "MLS")) %>%
  arrange(Treatment, RESISTANCE) %>%
  rename_genes() %>%
  select(sample, GENE, TPM) %>%
  tidyr::pivot_wider(
    names_from  = sample,
    values_from = TPM,
    values_fill = 0,
    values_fn   = sum
  ) %>%
  distinct(GENE, .keep_all = TRUE) %>%
  column_to_rownames("GENE")

geneTable_matrix <- data.matrix(geneTable_filtered, rownames.force = NA)
sum(is.na(geneTable_matrix))

# Clustering callback
callback <- function(hc, mat) {
  sv   <- svd(t(mat))$v[, 1]
  dend <- reorder(as.dendrogram(hc), wts = sv)
  as.hclust(dend)
}

# Min-max scale per column
geneTable_scaled <- apply(geneTable_matrix, 2, function(x) (x - min(x)) / (max(x) - min(x)))
range(geneTable_scaled)
rowSums(geneTable_scaled)

# Column annotation: treatment
geneTable_filtered_transposed <- as.data.frame(t(geneTable_filtered)) %>%
  mutate(sample = rownames(.)) %>%
  select(sample)

Metadata_Treatment <- genetable_normdata %>%
  select(sample, Treatment) %>%
  distinct()

ann_col <- inner_join(geneTable_filtered_transposed, Metadata_Treatment, by = "sample") %>%
  na.omit() %>%
  remove_rownames() %>%
  column_to_rownames(var = "sample")

# Row annotation: drug class
gene_meta <- genetable_normdata %>%
  filter(DATABASE == "card") %>%
  select(GENE, RESISTANCE) %>%
  rename_genes() %>%
  mutate(RESISTANCE = str_replace(
    RESISTANCE,
    "lincosamide;macrolide;streptogramin;streptogramin_A;streptogramin_B", "MLS"
  )) %>%
  distinct()

Resistance_category <- gene_meta %>%
  distinct(GENE, RESISTANCE, .keep_all = TRUE) %>%
  mutate(RESISTANCE = sapply(RESISTANCE, clean_string)) %>%
  mutate(RESISTANCE = sapply(RESISTANCE, classify_resistance)) %>%
  mutate(RESISTANCE = str_replace(RESISTANCE, "Mls", "MLS")) %>%
  column_to_rownames(var = "GENE") %>%
  rename(DRUG = RESISTANCE) %>%
  arrange(DRUG)

resistCategory <- as.factor(Resistance_category$DRUG)
r              <- length(levels(resistCategory))
r_palette      <- paletteer_d("ggsci::default_ucscgb", n = r)

ann_row_drug_col <- Resistance_category %>%
  select(DRUG) %>%
  na.omit() %>%
  distinct(DRUG) %>%
  mutate(color = r_palette)

drug_colors <- setNames(as.character(ann_row_drug_col$color), ann_row_drug_col$DRUG)

ann_colors <- list(
  Treatment = c("Reference diet" = "#1f77b4",
                "Dulce"          = "#2ca02c",
                "Soyabean meal"  = "#ff7f0e"),
  DRUG = drug_colors
)

# Pheatmap
heatmap_plot <- pheatmap(
  geneTable_scaled,
  display_numbers     = FALSE,
  cluster_cols        = FALSE,
  cluster_rows        = FALSE,
  scale               = "none",
  clustering_callback = callback,
  color               = colorRampPalette(c("#0612bd", "#bbbbbd", "#bd0606"))(100),
  border_color        = NA,
  show_rownames       = TRUE,
  annotation_row      = Resistance_category,
  annotation_col      = ann_col,
  annotation_colors   = ann_colors,
  fontsize_row        = 12,
  fontsize_col        = 12,
  fontsize            = 12
)

heatmap_plot
